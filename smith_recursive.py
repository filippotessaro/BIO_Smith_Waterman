...
def build_score_matrix(a, b, match_score=3, gap_cost=2):
    H = np.zeros((len(a) + 1, len(b) + 1), np.int)

    #Matrix looping -> itertools is for optimized cycles in matrices
    for i, j in itertools.product(range(1, H.shape[0]), range(1, H.shape[1])):
        #add the weight to 0 value store in each cell
        match = H[i - 1, j - 1] + (match_score if a[i - 1] == b[j - 1] else - match_score)
        delete = H[i - 1, j] - gap_cost #left
        insert = H[i, j - 1] - gap_cost #top
...
def traceback(H, b, b_='', old_i=0):
    ...
    #find the maximum cell score of the flipped matix
